...
def validation_sam(args, val_loader, epoch, net: nn.Module, clean_dir=True):
    ...
    n_val = len(val_loader)  # Number of batches in validation set
    mix_res = (0,) * 2  # For tracking IoU and Dice metrics
    tot = 0  # For tracking total loss
    threshold = (0.1, 0.3, 0.5, 0.7, 0.9)  # Thresholds for segmentation evaluation
    prompt_freq = args.prompt_freq  # Frequency of prompts

    lossfunc = criterion_G  # Loss function, e.g., BCEWithLogitsLoss or custom loss
    prompt = args.prompt  # Type of prompt (click or bbox)

    # Progress bar for validation
    with tqdm(total=n_val, desc='Validation round', unit='batch', leave=False) as pbar:
        for pack in val_loader:
            imgs_tensor = pack['image']  # Input images
            mask_dict = pack['label']  # Ground truth masks
            ...
            if len(imgs_tensor.size()) == 5:
                imgs_tensor = imgs_tensor.squeeze(0)
            frame_id = list(range(imgs_tensor.size(0)))  # List of frame indices

            # Initialize the state for validation
            train_state = net.val_init_state(imgs_tensor=imgs_tensor)

            # Frames to apply prompts based on prompt frequency
            ...
            for id in frame_id:
                obj_list += list(mask_dict[id].keys())
            obj_list = list(set(obj_list))  # Unique object list

            if len(obj_list) == 0:  # Skip if no objects
                continue

            # Inference with no gradients (evaluation mode)
            with torch.no_grad():
                # Add prompts (click or bbox) at specified frames
                for id in prompt_frame_id:
                    for ann_obj_id in obj_list:
                        try:
                            if prompt == 'click':
                            ...
                            # If no object, add an empty mask
