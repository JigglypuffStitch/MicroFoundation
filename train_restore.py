...
from sam2_train.modeling.utils_sr.utils import plot_img, move_to_cuda, load_checkpoint, save_checkpoint, \
    tensors_to_scalars, Measure, \
    get_all_ckpts
import clip
...
def main():
    ...
    if hparams['use_rrdb']:
            ...
            rrdb = None
    text_encoder, _ = clip.load("ViT-B/32", device=device)
    text_encoder.eval()
    ...
    train_dataset = SuperResolutionDataset(args, args.data_path, args.data_path, args.target_dir,
                                                      transform=transform_train, transform_gt=transform_gt,
                                                      mode='train')
    test_dataset = SuperResolutionDataset(args, args.data_path, args.data_path, args.target_dir,
                                                     transform=transform_test, transform_gt=transform_gt, mode='test')

    nice_train_loader = DataLoader(train_dataset, batch_size=args.b, shuffle=True, num_workers=2,
                                       pin_memory=True)
    nice_test_loader = DataLoader(test_dataset, batch_size=args.b, shuffle=False, num_workers=2,
                                      pin_memory=True)
    ...
    for epoch in range(settings.EPOCH):
        ...
        for epoch in range(settings.EPOCH):
                ...
                scheduler.step(training_step * len(train_dataset))
                ...
                if epoch % args.val_freq == 0 and epoch != 0:
                    ...
                    if metrics['psnr'] > best_PSNR:
                            ...
                            save_checkpoint(diffusion, optimizer, '/home/cli348/New/logs', epoch)
        ...
        if epoch % args.val_freq == 0 and epoch != 0:
            ...
            if metrics['psnr'] > best_PSNR:
                    best_PSNR = metrics['psnr']
                    save_checkpoint(diffusion, optimizer, '/home/cli348/New/checkpoints/', epoch)
...
def validate():
    ...
    training_step = load_checkpoint(diffusion, optimizer,"/home/cli348/New/checkpoints/255/Microtubules", 1)
    ...
    nice_train_loader = DataLoader(refuge_train_dataset, batch_size=args.b, shuffle=True, num_workers=2,
                                       pin_memory=True)
    nice_test_loader = DataLoader(refuge_test_dataset, batch_size=args.b, shuffle=False, num_workers=2,
                                      pin_memory=True)
    '''checkpoint path and tensorboard'''
    checkpoint_path = os.path.join(settings.CHECKPOINT_PATH, args.net, settings.TIME_NOW)
    # use tensorboard
