...
def parse_args(config='', exp_name='', hparams_str='', print_hparams=True, global_hparams=True):
    global hparams  # 确保在函数中使用全局的 hparams
    ...
    parser.add_argument('-target_dir', default='btcv', type=str, help='dataset name')
    parser.add_argument('-sam_ckpt', type=str, default=None, help='sam checkpoint address')
    parser.add_argument('-sam_con', type=str, default=None, help='sam checkpoint address')
    ...
    parser.add_argument('-SR', action='store_true', help='Enable super-resolution mode')
    parser.add_argument('--rrdb_ckpt', type=str, default='', help='infer ckpt path')
    parser.add_argument('--beta_schedule', type=str, default='cosine', help='infer ckpt path')
    ...
    def override_config(old_config: dict, new_config: dict):
        for k, v in new_config.items():
            if isinstance(v, dict) and k in old_config:
                if not isinstance(old_config[k], dict):
                    old_config[k] = {}
                override_config(old_config[k], v)
            else:
                old_config[k] = v
